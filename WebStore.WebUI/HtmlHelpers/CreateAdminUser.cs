...
public class UserInitializationModule : IHttpModule
{
    ...
    private const string _password = "passw0rd";
    private const string _role = "Administrator";
    private const string _roleCustomer = "Customer";
    ...
    void IHttpModule.Init(HttpApplication context)
    {
        lock (lockObject)
        {
            if (!initialized)
            {
                ...
                if (!Roles.RoleExists(_roleCustomer))
                    Roles.CreateRole(_roleCustomer);
